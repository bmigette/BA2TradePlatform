...
from .fundamental import fundamental_score, piotroski_f_score, altman_z
from .analyst import revision_momentum
from .macro import (trend_score, vix_score, pmi_score, sahm_score,
                    yield_curve_score, regime_composite, DEF_MW)
...
class DeterministicScorer(AnalysisStatusRenderMixin, FMPApiKeyMixin, MarketExpertInterface):
    ...
    def get_settings_definitions(cls) -> Dict[str, Any]:
        return {
            ...
            "min_score_delta": {"type": "float", "required": False, "default": 0.0,
                                "description": "Extra score margin required to re-enter after the opposite signal (churn filter)"},
            ...
            "z_veto": {"type": "float", "required": False, "default": 1.8,
                       "description": "Altman Z below this = hard distress veto"},
            ...
            "fundamentals_max_age_days": {"type": "int", "required": False, "default": 180,
                                          "description": "Ignore fundamentals older than this (0 = off)"},
    ...
    _SETTING_KEYS = (
        ...
        "k_compress", "theta_buy", "theta_sell", "min_score_delta", "veto_cap",
        ...
        "z_veto", "fscore_disqualify", "scale_accel", "fundamentals_max_age_days",
    ...
    def _process(self, data_bundle: Dict[str, Any], settings: Dict[str, Any],
        ...
        # ---- FUNDAMENTAL ----
        fund = self._build_fundamental(data_bundle, settings)
        ...
        final = result["final"]
        prev_signal = None  # stateless per-bar verdict; hysteresis handled by platform
    ...
    def _build_fundamental(self, data_bundle: Dict[str, Any],
        ...
        if len(revs) >= 3:
            try:
                ...
                qoq = [vals[i] / vals[i - 1] - 1.0 for i in range(1, len(vals)) if vals[i - 1]]
                if len(qoq) >= 2:
                    snapshot["rev_accel"] = qoq[-1] - float(sum(qoq[:-1]) / len(qoq[:-1]))
    ...
    def _build_regime(self, data_bundle: Dict[str, Any],
        ...
        inputs = {
            ...
            "credit": None,   # needs OAS series (v2)
            "yield_curve": yield_curve_score(
                mi.get("spread_10y3m_series") if isinstance(mi.get("spread_10y3m_series"), object) else
                (None if mi.get("spread_10y3m") is None else _scalar_series(mi["spread_10y3m"])),
                scale=float(settings.get("yc_scale", 0.005))),
    ...
    def render_market_analysis(self, market_analysis: MarketAnalysis) -> str:
        ...
        if not block:
            return super().render_market_analysis(market_analysis) if hasattr(super(), "render_market_analysis") else ""
        rec = block.get("recommendation", {})
        scores = block.get("scores", {})
        levels = block.get("levels", {})
        lines = [
            f"**DeterministicScorer** — {rec.get('signal')} "
            f"(confidence {rec.get('confidence', 0):.0f}%)",
            f"final score {scores.get('final', 0):+.3f} = "
            f"T {scores.get('technical'):+.2f} · F {scores.get('fundamental'):+.2f} · "
            f"A {(scores.get('analyst') or 0):+.2f} · regime {(scores.get('regime') or 0):+.2f}",
        ]
        if scores.get("veto"):
            lines.append("⚠️ fundamental distress veto applied")
        if levels.get("target_price"):
            lines.append(f"target {levels['target_price']:.2f} · "
                         f"stop {levels.get('stop_price'):.2f} · ATR {levels.get('atr'):.2f}")
        return "\n".join(lines)
