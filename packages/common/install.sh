...
OWNER="bmigette"
HERE="$(cd "$(dirname "$0")/.." && pwd)"            # dev dir holding the sibling clones
BASE="${BASE:-$HOME}"
...
install_chain() {  # $1=uv $2=vpy ; --no-sources so our explicit chain install beats the git pins
  ...
  if [ "$EDITABLE" = "1" ]; then
    common="$HERE/BA2TradeCommon"; prov="$HERE/BA2TradeProviders"
    exp="$HERE/BA2TradeExperts"; [ "$UI" = "1" ] && exp="${exp}[ui]"
  else
...
if [ "$DO_TRADE" = "1" ]; then
  echo "==== TRADE venv ===="
  new_app_venv "$VENV_ROOT/trade" "$HERE/BA2TradePlatform" \
               "$HERE/BA2TradePlatform/requirements.txt" 1 "ba2_common" "$TRADE_PY"
fi
if [ "$DO_TEST" = "1" ]; then
  echo "==== TEST venv ===="
  new_app_venv "$VENV_ROOT/test" "$HERE/BA2TestPlatform" \
               "$HERE/BA2TestPlatform/backend/requirements.txt" 0 "ba2_common" "$TEST_PY"
fi
