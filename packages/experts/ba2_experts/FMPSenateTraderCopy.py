...
from ba2_common.core.models import ExpertInstance, MarketAnalysis, AnalysisOutput, ExpertRecommendation
from ba2_common.core.db import get_db, get_instance, update_instance, add_instance
...
class FMPSenateTraderCopy(AnalysisStatusRenderMixin, FMPCongressTradingMixin, MarketExpertInterface):
    ...
    # FMPSenateTraderCopy analyzes a BASKET (all symbols traded by the followed
    # traders), so _process returns List[Recommendation] (one per supported
    # symbol) — the engine writes one ExpertRecommendation row per element, as
    # the live run_analysis does. _gather fetches all latest senate/house
    # disclosures and pre-resolves the per-symbol current price (current_price_map:
    # the account/broker quote per symbol when as_of=None live, else the OHLCV
    # close-at-as_of) + the set of supported symbols (those with a resolvable
    # price), so _process never touches a provider/account. With as_of=None the
    # fetch is the live latest (byte-identical); when as_of is set, _process
    # additionally drops any trade whose disclosureDate/transactionDate is after
    # as_of (no-lookahead).
    #
    # The followed-trader names and age thresholds are SETTINGS resolved before
    # _process and passed via the settings dict (copy_trade_names / _subtype).
    # ------------------------------------------------------------------
    _SETTING_KEYS = ("copy_trade_names", "max_disclose_date_days", "max_trade_exec_days")
    ...
    @staticmethod
    def _trade_date_visible(trade: Dict[str, Any], ceiling: datetime) -> bool:
        """No-lookahead gate: a trade is only knowable once BOTH its disclosure and
        execution dates are on/before the as_of ceiling. Rows with unparseable
        dates are dropped (the multi-symbol age filter requires both dates)."""
        for key in ('disclosureDate', 'transactionDate'):
            ds = trade.get(key, '')
            if not ds:
                return False
            try:
    ...
    def _process(self, data_bundle: Dict[str, Any], settings: Dict[str, Any],
        ...
        subtype = settings.get("_subtype") or AnalysisUseCase.ENTER_MARKET
        max_disclose_days = int(settings["max_disclose_date_days"])
        max_exec_days = int(settings["max_trade_exec_days"])
        copy_trade_names = [
            n.strip().lower() for n in str(settings.get("copy_trade_names", "")).split(",")
            if n.strip()
        ]

        all_trades = list(data_bundle["senate_trades"]) + list(data_bundle["house_trades"])
        # No-lookahead: when as_of is set, drop trades not yet visible at as_of.
        if as_of is not None:
            all_trades = [t for t in all_trades if self._trade_date_visible(t, now)]

        filtered_trades = self._filter_trades_by_age_multi(
            all_trades, max_disclose_days, max_exec_days, now=now)
        copy_trades = self._find_copy_trades(filtered_trades, copy_trade_names)

        # Group copy trades by symbol, restricted to broker-supported symbols.
        supported = data_bundle["supported_symbols"]
        current_price_map = data_bundle["current_price_map"]
        trades_by_symbol: Dict[str, List[Dict[str, Any]]] = {}
        for trade in copy_trades:
            ts = str(trade.get('symbol', '')).upper().strip()
            if ts and ts in supported:
                trades_by_symbol.setdefault(ts, []).append(trade)

        recommendations: List[Recommendation] = []
        for trade_symbol, symbol_trades in sorted(trades_by_symbol.items()):
            current_price = current_price_map.get(trade_symbol)
            if not current_price:
                continue
            rec_data = self._generate_recommendations(
                symbol_trades, trade_symbol, current_price, subtype=subtype,
                all_copy_trades=copy_trades)
            recommendations.append(Recommendation(
                signal=rec_data['signal'], confidence=round(rec_data['confidence'], 1),
                current_price=current_price, details=rec_data['details'],
                expected_profit_percent=rec_data['expected_profit_percent'],
                raw_outputs={"name": "Copy Trade Analysis", "type": "copy_trade_analysis",
                             "text": rec_data['details'], "symbol": trade_symbol,
                             "recommendation": rec_data}))
        return recommendations

    def analyze_as_of(self, as_of: datetime, context: BacktestContext) -> List[Recommendation]:
        """BacktestInterface entry for the basket expert: returns a List[Recommendation]
        (one per supported symbol). Runs the SAME _gather+_process as live."""
        merged = dict(context.settings)
        if "_subtype" not in merged:
            merged["_subtype"] = context.subtype or AnalysisUseCase.ENTER_MARKET
    ...
    def _filter_trades(self, trades: List[Dict[str, Any]],
        ...
        Args:
            ...
            now: reference 'now' for age math (as_of in backtest); defaults to
...
Recommendations Generated:"""
    ...
    def _run_enter_market_analysis(self, symbol: str, market_analysis: MarketAnalysis) -> None:
        ...
        try:
            ...
            copy_trade_names_setting = (settings.get('copy_trade_names') or '').strip()
            ...
            max_disclose_days = int(settings['max_disclose_date_days'])
            max_exec_days = int(settings['max_trade_exec_days'])
            settings['copy_trade_names'] = copy_trade_names_setting
            settings['_subtype'] = AnalysisUseCase.ENTER_MARKET

            # Thin orchestrator: _gather(as_of=None) + _process — the EXACT same pair
            # the backtest engine drives via analyze_as_of. _gather fetches all latest
            # disclosures and pre-resolves the per-symbol current price + supported set
            # (live as_of=None uses the account/broker quote per symbol; supported =
            # symbols with a resolvable price, mirroring the live broker filter).
            providers = self._live_providers()
            bundle = self._gather(providers, as_of=None)
            senate_trades = bundle["senate_trades"]
            house_trades = bundle["house_trades"]
            if senate_trades == [] and house_trades == []:
                raise ValueError("Failed to fetch trades from FMP API (both senate and house empty)")
            all_trades = list(senate_trades) + list(house_trades)

            self.logger.info(f"Fetched {len(senate_trades)} senate trades and "
                       f"{len(house_trades)} house trades for all symbols")

            recommendations = self._process(bundle, settings, as_of=None)

            # Recompute filtered/copy trades for the analysis-output records (same
            # logic _process applies; cheap and keeps the persisted outputs intact).
            filtered_trades = self._filter_trades_by_age_multi(
            ...
            for trade in copy_trades:
                ts = str(trade.get('symbol', '')).upper().strip()
                if ts and ts in bundle["supported_symbols"]:
                    trades_by_symbol.setdefault(ts, []).append(trade)

            self.logger.info(f"Found copy trades for {len(trades_by_symbol)} supported symbols: "
                       f"{sorted(trades_by_symbol.keys())}")

            # Persist one ExpertRecommendation per element of the basket result.
            recommendation_ids = []
            ...
            for rec in recommendations:
                ...
                try:
                    ...
                    symbol_recommendations[trade_symbol] = {
                        ...
                        'trade_count': len(trades_by_symbol.get(trade_symbol, [])),
            ...
            self.logger.info(f"Completed FMPSenateTraderCopy analysis for {symbol}: "
                       f"{len(recommendation_ids)} recommendation(s) across "
                       f"{len(trades_by_symbol)} symbol(s)")
