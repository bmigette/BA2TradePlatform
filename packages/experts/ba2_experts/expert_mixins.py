...
class FMPCongressTradingMixin(FMPApiKeyMixin):
    ...
    def _fetch_congress_trades(
        ...
        try:
            ...
            # ``-latest`` (all-symbols, recency) list — which FMPSenateTraderCopy fetches once per
            # universe symbol per bar — is cached under a single fixed per-chamber key so it is
            # fetched once per worker instead of (universe size) times per bar.
