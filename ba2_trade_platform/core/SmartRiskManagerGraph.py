...
from .SmartRiskManagerPrompts import (
...
class SmartRiskManagerGraph:
    ...
    def _create_research_tools(self) -> List:
        ...
        def get_all_recent_analyses_tool(
            max_age_hours: Annotated[Optional[int], "Max age in hours (default: 24). Pass a larger value to pull older analyses for CONTEXT only — trading is restricted to symbols analyzed in the last 24h."] = None
        ) -> List[Dict[str, Any]]:
            """Get ALL recent market analyses across all symbols.

            Defaults to the 24h tradable window. A larger max_age_hours returns
            older analyses for context/research only; positions may only be OPENED
    ...
    def _initialize_research_agent(self, state: SmartRiskManagerState) -> Dict[str, Any]:
        ...
        try:
            ...
            # Direction policy: respect enable_buy/enable_sell so the agent does not
            # waste an iteration proposing a trade the toolkit will reject. Injected via
            # the {expert_instructions} slot to avoid touching the shared prompt signature.
            direction_rules = []
            if not expert.get_setting_with_interface_default("enable_buy"):
                direction_rules.append(
                    "- BUY orders are DISABLED for this expert — do NOT call recommend_open_buy_position."
                )
            if not expert.get_setting_with_interface_default("enable_sell"):
                direction_rules.append(
                    "- SHORT/SELL-to-open is DISABLED for this expert — do NOT call recommend_open_sell_position. "
                    "(Closing or reducing existing long positions is still allowed.)"
                )
            if not direction_rules:
                # Always emit the section (RM-7) so the prompt's "Direction Policy"
                # references never point at a missing heading.
                direction_rules.append(
                    "- BUY and SELL-to-open are both ENABLED — size by conviction within the position limits."
                )
            direction_policy = "\n\n## DIRECTION POLICY\n" + "\n".join(direction_rules)

            formatted_expert_instructions = (
                f"{direction_policy}\n\n{expert_instructions}" if expert_instructions
                else direction_policy
            )
