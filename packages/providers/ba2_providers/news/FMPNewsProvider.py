...
from ba2_common.core.provider_utils import calculate_date_range, log_provider_call
...
class FMPNewsProvider(MarketNewsInterface):
    ...
    def get_company_news(
        ...
        try:
            ...
            # Through fmp_list_call so this shares the global FMP rate-limit gate (a bare call
            # would bypass it and storm the limit under the parallel GA grid).
            news_data = fmp_list_call(
                lambda: fmpsdk.stock_news(
                    apikey=self.api_key,
                    tickers=symbol,
