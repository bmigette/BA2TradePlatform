...
# --- Effective-date helpers (Phase 1 native as_of cache, Decision 7) ----------
# Reporting-lag fallbacks — documented approximations used only when the SEC
# publication date is missing on a raw provider row.
...
def parse_provider_date(value, default=None):
    ...
    Returns ``default`` for empty/unparseable input. The date part only is kept
    (FMP timestamps vary in precision); the result is anchored at UTC midnight so
    effective_date<=as_of comparisons are stable regardless of input precision.
    """
...
def insider_effective_date(txn: dict):
    """Form-4 effective (publication) date: filingDate else transactionDate + lag.
...
def statement_effective_date(row: dict):
    """Statement effective (publication) date: fillingDate/filingDate/acceptedDate
    else fiscalDateEnding (or 'date') + reporting-lag.
