...
class _LazyNiceGuiUI:
    """Lazy proxy for ``nicegui.ui``.

    nicegui is an OPTIONAL extra of ba2_experts ([ui]). Importing this module
    must not require nicegui to be installed, so ``ui`` is a proxy that imports
    ``nicegui.ui`` on first attribute access (i.e. only when something actually
    renders). All usages in this module are ``ui.<attr>`` attribute access.
    """
