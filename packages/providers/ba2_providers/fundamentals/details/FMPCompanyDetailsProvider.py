...
class FMPCompanyDetailsProvider(CompanyFundamentalsDetailsInterface):
    ...
    def get_balance_sheet(
        ...
        try:
            # FMP balance_sheet_statement returns list of balance sheets (full per-symbol
            # history; the filing-date <= as_of filtering below is unchanged and as_of-independent).
            # BACKTEST-ONLY disk cache keyed by symbol AND period so annual vs quarterly histories
            # never collide on one cache file; fmp_list_call adds the rate gate.
            period = "annual" if frequency == "annual" else "quarter"
            balance_data = fmp_history_disk_cached(
                f"balance_sheet_{period}", symbol,
                lambda: fmp_list_call(
            ...
            filtered_statements = self._filter_statements_by_date(
                balance_data, end_date, start_date, lookback_periods, as_of
            )
    ...
    def get_income_statement(
        ...
        try:
            # FMP income_statement returns list of income statements (full per-symbol history;
            # the filing-date <= as_of filtering below is unchanged and as_of-independent).
            # BACKTEST-ONLY disk cache keyed by symbol AND period so annual vs quarterly histories
            # never collide on one cache file; fmp_list_call adds the rate gate.
            period = "annual" if frequency == "annual" else "quarter"
            income_data = fmp_history_disk_cached(
                f"income_statement_{period}", symbol,
                lambda: fmp_list_call(
            ...
            # Filter by date if needed
            filtered_statements = self._filter_statements_by_date(
                income_data, end_date, start_date, lookback_periods, as_of
            )
    ...
    def get_cashflow_statement(
        ...
        try:
            # FMP cash_flow_statement returns list of cash flow statements (full per-symbol
            # history; the filing-date <= as_of filtering below is unchanged and as_of-independent).
            # BACKTEST-ONLY disk cache keyed by symbol AND period so annual vs quarterly histories
            # never collide on one cache file; fmp_list_call adds the rate gate.
            period = "annual" if frequency == "annual" else "quarter"
            cashflow_data = fmp_history_disk_cached(
                f"cashflow_statement_{period}", symbol,
                lambda: fmp_list_call(
            ...
            # Filter by date if needed
            filtered_statements = self._filter_statements_by_date(
                cashflow_data, end_date, start_date, lookback_periods, as_of
            )
    ...
    def _filter_statements_by_date(
        ...
        When ``as_of`` is set (LOOKAHEAD BUG FIX), statements are first restricted to
        those whose SEC effective date (``fillingDate``/``filingDate``/``acceptedDate``,
        or ``fiscalDateEnding`` + reporting-lag fallback) is on or before ``as_of`` —
        a statement is only knowable once it has been filed/accepted, not on its fiscal
        period-end date. When ``as_of`` is ``None`` (live) this pre-pass is skipped and
        the original ``fiscalDateEnding`` slicing applies byte-for-byte.
        """
        if not statements:
            return []

        # No-lookahead anchor: drop statements not yet filed as of `as_of`. Compare both
        # sides tz-naive: statement_effective_date may return an aware datetime while the
        # caller's as_of is naive (backtest/screener), which otherwise raises
        # "can't compare offset-naive and offset-aware datetimes".
        if as_of is not None:
            as_of_cmp = as_of.replace(tzinfo=None) if as_of.tzinfo is not None else as_of
            kept = []
            for r in statements:
                eff = statement_effective_date(r)
                if eff is None:
                    continue
                eff_cmp = eff.replace(tzinfo=None) if eff.tzinfo is not None else eff
                if eff_cmp <= as_of_cmp:
                    kept.append(r)
            statements = kept
            if not statements:
                return []
    ...
    def get_past_earnings(
        ...
        try:
            ...
            # Fetch earnings data using FMP SDK (full per-symbol history; the lookback/end_date
            # filtering below is unchanged). BACKTEST-ONLY disk cache keyed by symbol AND frequency
            # so annual vs quarterly histories never collide on one cache file; spawned GA workers
            # read it from disk instead of re-fetching; fmp_list_call adds the gate.
            # NOTE: fmpsdk.historical_earning_calendar defaults to limit=10, i.e. only the ~10 most
            # recent periods (≈2.5y, incl. future estimated rows). That silently returned ZERO
            # earnings for any as-of/backtest window older than ~now-2.5y (the as-of filter below
            # found nothing before the truncated window) — e.g. an EarningsDrift backtest of
            # Feb-2024 run in 2026. FMP itself serves full history (AAPL = 164 rows back to 1985),
            # so request a large limit and let the end_date + lookback_periods filtering below trim
            # it. Cached per (symbol, frequency), so the full history is fetched once.
            earnings_data = fmp_history_disk_cached(
                f"past_earnings_{frequency}", symbol,
                lambda: fmp_list_call(
                    lambda: fmpsdk.historical_earning_calendar(
                        apikey=self.api_key,
                        symbol=symbol,
                        limit=_PAST_EARNINGS_FETCH_LIMIT,
                    ),
                    symbol=symbol,
    ...
    def get_earnings_estimates(
        ...
        try:
            ...
            def _fetch_estimates():
                # Through fmp_http_get: shares the global FMP rate-limit gate (a raw requests.get
                # would bypass it and storm the limit under the parallel GA grid) + handles the
                # 200-with-error-body case. Raises on an FMP error/unexpected payload so the disk
                # cache NEVER persists an error (it only caches a truthy, valid result) and the
                # call is retried next run.
                response = fmp_http_get(url, params, symbol=symbol,
                                        endpoint="analyst-estimates", timeout=30)
                data = response.json()
                # FMP returns rate-limit / API errors as HTTP 200 with a JSON dict
                # (e.g. {"Error Message": "Limit Reach."}).
                if isinstance(data, dict):
                    ...
                    if err:
                        logger.error(
                            f"FMP analyst-estimates error for {symbol}. Payload: {data!r}"
                        )
                        raise FMPError(f"FMP analyst-estimates error for {symbol}: {err}")
                    logger.error(
                        f"FMP analyst-estimates unexpected payload for {symbol}. Payload: {data!r}"
                    )
                    raise FMPError(
            ...
            # BACKTEST-ONLY disk cache keyed by (namespace, symbol): analyst-estimates was the ONE
            # FactorRanker/PEAD input not disk-cached (earnings/balance/income/cashflow/grades are),
            # so it re-fetched over the network per symbol per rebalance — profiled as ~96% of a
            # FactorRanker forward/optimization run's wall time (SSL/connect + rate-limit sleeps).
            # Cached, spawned GA workers read the full history from disk once. Live path (TTL not
            # frozen) is an unchanged passthrough to the fetch above.
            estimates_data = fmp_history_disk_cached(
                f"earnings_estimates_{frequency}", symbol, _fetch_estimates,
            )
    ...
    def get_financial_ratios(self, symbol, as_of_date, format_type="markdown"):
        ...
        scores = {}
        try:
            # Through fmp_http_get so this shares the global FMP rate-limit gate (a raw
            # requests.get would bypass it and storm the limit under the parallel GA grid).
            resp = fmp_http_get("https://financialmodelingprep.com/api/v4/score",
                                {"symbol": sym, "apikey": self.api_key},
                                symbol=sym, endpoint="v4/score", timeout=30)
            if resp.ok:
                js = resp.json()
