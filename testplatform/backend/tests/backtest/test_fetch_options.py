...
def test_contract_to_metadata_chain_row_has_no_greeks_and_no_quotes_without_premium():
    ...
    row = contract_to_metadata_chain_row(c, "AAPL", risk_free_rate=0.05)
    assert row["occ_symbol"] == "AAPL240315C00180000"
...
def test_contract_to_metadata_chain_row_fills_quotes_from_as_of_premium():
    ...
    c = _Contract("AAPL240315C00180000", "call", "180", "2024-03-15")
    row = contract_to_metadata_chain_row(c, "AAPL", as_of_premium=4.6, risk_free_rate=0.05)
...
def test_the_rate_is_a_required_keyword_of_both_mappers():
    """No default rate: the builder states the as-of DGS3MO rate for every row it inverts."""
    import pytest

    class B: open=2.0; high=2.5; low=1.9; close=2.3; volume=100
    with pytest.raises(TypeError):
        bar_to_row("AAPL240315C00180000", "2024-03-05", B(), "AAPL", "call", 180.0, "2024-03-15")
    with pytest.raises(TypeError):
        contract_to_metadata_chain_row(
            _Contract("AAPL240315C00180000", "call", "180", "2024-03-15"), "AAPL")
...
def test_contract_to_metadata_chain_row_accepts_string_type_and_expiry():
    ...
    row = contract_to_metadata_chain_row(_Plain(), "MSFT", risk_free_rate=0.05)
