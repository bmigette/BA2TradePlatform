...
from ba2_common.core.interfaces import MarketIndicatorsInterface
from ba2_common.core.interfaces.MarketDataProviderInterface import MarketDataProviderInterface
from ba2_common.core.provider_utils import validate_date_range, log_provider_call
...
class PandasIndicatorCalc(MarketIndicatorsInterface):
    ...
    def _calculate_indicator_for_range(
        ...
        # Get price data via data provider (uses smart cache). Fetch up to the REQUESTED
        # end_date (NOT wall-clock now): a backtest passes a historical as_of, and fetching
        # to now() would leak future bars into the (causal) indicator at that as_of. Live
        # callers pass end_date=now(), so this is unchanged for them.
