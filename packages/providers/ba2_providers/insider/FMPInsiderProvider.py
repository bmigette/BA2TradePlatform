...
from ba2_providers.fmp_common import fmp_http_get, fmp_history_disk_cached
from ba2_common.core.interfaces import CompanyInsiderInterface
...
class FMPInsiderProvider(CompanyInsiderInterface):
    ...
    # FMP returns Form-4 insider rows newest-first, 100 per page. ``fmpsdk.insider_trading``
    # only ever returns page 0 (the latest ~14 months) and ignores ``limit`` — so any backtest
    # whose as_of predates that window saw ZERO insider rows for every symbol. The v4 endpoint's
    # ``page`` param is the only way to reach history, so paginate it into the full per-symbol
    # series (which is then date/as_of-filtered downstream, and disk-cached per symbol).
    _INSIDER_V4_URL = "https://financialmodelingprep.com/api/v4/insider-trading"
    _INSIDER_MAX_PAGES = 20  # safety cap; ~100 rows/page, stops early on the last (short) page

    def _fetch_insider_history(self, symbol: str) -> list:
        """Paginate FMP v4 insider-trading into the full per-symbol history (newest-first).

        Returns ``[]`` on no data (never cached, so it retries next run). Window-independent —
        always fetches the same full series for a symbol, so the per-symbol disk cache stays
        correct regardless of the requested as_of/date range."""
        rows: list = []
        for page in range(self._INSIDER_MAX_PAGES):
            resp = fmp_http_get(
                self._INSIDER_V4_URL,
                params={"symbol": symbol, "page": page, "apikey": self.api_key},
                symbol=symbol, endpoint="insider-trading",
            )
            batch = resp.json()
    ...
    def get_insider_transactions(
        ...
        Args:
            ...
            as_of: Point-in-time anchor (LOOKAHEAD FIX). When set, a transaction is
                only included once its Form-4 ``filingDate`` (effective/publication
                date) is on or before ``as_of`` — ``transactionDate`` remains the
                value/display date and the window bound. When ``None`` (live) the
                behaviour is byte-identical to before: filtering keys solely on the
        ...
        try:
            # Full per-symbol insider history (paginated v4 fetch above), behind a BACKTEST-ONLY
            # disk cache keyed by symbol — the fetch is as_of-independent (the date-range /
            # no-lookahead filtering below is unchanged), so a per-symbol cache is safe and lets
