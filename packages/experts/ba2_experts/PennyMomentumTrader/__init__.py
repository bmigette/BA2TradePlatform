...
from ba2_common.core.interfaces import LiveExpertInterface
...
from ba2_experts.PennyMomentumTrader.conditions import validate_condition_set
from ba2_experts.PennyMomentumTrader.prompts import build_conditions_fix_prompt
from ba2_experts.PennyMomentumTrader.trade_manager import PennyTradeManager
...
class PennyMomentumTrader(
    ...
    def render_market_analysis(self, market_analysis):
        from ba2_experts.PennyMomentumTrader.ui import PennyMomentumTraderUI
    ...
    def _on_pipeline_error(self, exc: Exception) -> None:
        """Mark the current MarketAnalysis as FAILED and log the crash."""
        try:
            ma = self._get_todays_market_analysis()
            if ma is None:
                return
            from sqlalchemy.orm import attributes
            with get_db() as session:
                db_ma = session.get(MarketAnalysis, ma.id)
                if db_ma and db_ma.status == MarketAnalysisStatus.RUNNING:
                    state = db_ma.state or {}
                    state["pipeline_error"] = str(exc)
                    state["pipeline_failed_phase"] = getattr(self, "_current_phase", "unknown")
                    db_ma.state = state
                    db_ma.status = MarketAnalysisStatus.FAILED
                    attributes.flag_modified(db_ma, "state")
                    session.add(db_ma)
                    session.commit()
                    self.logger.error(
                        f"MarketAnalysis id={ma.id} marked FAILED "
                        f"(phase: {state['pipeline_failed_phase']}): {exc}"
                    )
        except Exception as e:
            self.logger.warning(f"_on_pipeline_error: failed to mark MA as failed: {e}")
        ...
        try:
            ...
            log_activity(
                ...
                description=(
                    f"PennyMomentumTrader pipeline crashed at phase "
                    f"'{getattr(self, '_current_phase', 'unknown')}': {exc}"
                ),
                data={"error": str(exc), "phase": getattr(self, "_current_phase", "unknown")},
