...
AS_OF LIMITATION: this provider's ``as_of_date`` parameter is plumbed for the API
shape but the underlying FMP profile / TTM ratio endpoints return a "now" snapshot
regardless of the requested date. True point-in-time reconstruction requires the
effective-date cache and is a documented limitation (deferred); for backtest use the
statement (``FMPCompanyDetailsProvider``) effective-date path instead.
"""
...
from ba2_common.core.interfaces import CompanyFundamentalsOverviewInterface
