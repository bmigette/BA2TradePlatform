...
DEF_Z_VETO = 1.8          # Altman distress zone
DEF_Z_GREY = 2.99
...
def altman_z(cur: Dict[str, Any], market_cap: Optional[float],
    ...
    """Altman Z-Score. `variant`: original (manufacturers), adjusted (Z'' for
...
def growth_acceleration(history: List[Optional[float]], scale: float = DEF_SCALE_ACCEL) -> Optional[float]:
    """QoQ growth of the latest quarter vs the trailing-4Q average QoQ growth.

    `history` = ascending quarterly values (revenue or EPS), needs >= 6 points.
    """
    vals = [v for v in history if v is not None and math.isfinite(v)]
    if len(vals) < 6:
        return None
    qoq = [vals[i] / vals[i - 1] - 1.0 for i in range(1, len(vals)) if vals[i - 1] not in (0, None)]
    if len(qoq) < 5:
        return None
    latest = qoq[-1]
    trail = float(np.mean(qoq[-5:-1]))
    return latest - trail  # caller tanh-scales
...
def fundamental_score(snapshot: Dict[str, Any], s: Dict[str, Any]) -> Dict[str, Any]:
    ...
    score = (sum(w * v for w, v in components.values()) / total_w) if total_w > 0 else 0.0
