...
async def track_client_ip(request, call_next):
    ...
    try:
        from .pages.server_performance import active_clients, clients_lock, get_country_from_ip
        import time
        ...
        # Track this client with country information
        with clients_lock:
            ...
            logger.debug(f"Tracked client IP: {client_ip} -> Country: {country} (active_clients now has {len(active_clients)} entries)")
    except Exception as e:
...
@ui.page('/serverperf')
def server_performance_page() -> None:
