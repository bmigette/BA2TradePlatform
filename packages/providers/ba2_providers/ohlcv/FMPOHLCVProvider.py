...
class FMPOHLCVProvider(MarketDataProviderInterface):
    ...
    def _fetch_intraday_data(
        ...
        # FMP caps each intraday response at ~624 bars (~8 trading days) ending at `to`: a single
        # wide request therefore returns ONLY the tail of the range. Chunk into <=8-calendar-day
        # windows (<=~6 trading days, safely under the cap) and concatenate so we get full history.
        import os as _os
        ...
        chunk_days = 8
        # Build the <=8-day (from,to) windows up front, then fetch them CONCURRENTLY: FMP caps each
        # call at ~624 bars so deep history needs many small calls — parallelising turns a long
        # serial chain into a fan-out. fmp_http_get's backoff absorbs the rate-limit pressure.
        windows: list = []
        win_start = _day(start_date)
        end_day = _day(end_date)
        while win_start <= end_day:
            win_end = min(win_start + _td(days=chunk_days), end_day)
            windows.append((win_start, win_end))
            win_start = win_end + _td(days=1)
        ...
        def _fetch_window(w):
            ...
            resp = fmp_http_get(url, params=params, symbol=symbol, endpoint="historical-chart")
            chunk = resp.json()
        ...
        logger.debug(f"FMP intraday {symbol} {fmp_interval}: {len(windows)} chunked call(s) "
                     f"({max_workers if windows else 0} parallel), {len(records)} raw bars")
        if not records:
        ...
        # Sort by date (FMP returns newest first) + dedup any chunk-boundary overlap
