...
class LiveExpertInterface(MarketExpertInterface):
                ...
                if not self._stop_event.is_set():
                    try:
                        ...
                        try:
                            ...
                            _log.warning(
                                f"LiveExpert {self.id}: _on_pipeline_error hook raised: {hook_exc}"
                            )
                    finally:
                        self._resume_mode = False
    ...
    def _on_pipeline_error(self, exc: Exception) -> None:
        """Called when _run_daily_pipeline raises an unhandled exception.

        Override in subclasses to update MarketAnalysis status, log activity
        entries, or perform any other cleanup specific to the pipeline's state.
