...
- **Breaker TRANSITIONS and the exit/servicing pass: LIVE ONLY.** `option_lifecycle_service` — the only caller of `update_breaker` — runs from `JobManager`. A backtest therefore never trips or clears the latch: a `classic_options` backtest models **entry gating only**, and its exits come from the strategy's own `close_option` rules. Do not read a backtest as evidence about the breaker or about profit-capture/roll-DTE/tested-delta behaviour. Wiring the backtest to the same shared transition is agreed work, blocked on one question (what the option sleeve's equity IS per bar — live `get_balance()` returns account equity, the backtest's returns spendable cash); see the LIMITATION note in `ba2_common/core/OptionRiskManagement.py`.
- **The lifecycle thresholds are declared by no expert.** `circuit_breaker_pct`, `profit_capture_pct`, `roll_dte`, `tested_delta_enabled`, `dr_stop_enabled` and `ur_stop_enabled` left the tree with PremiumSeller's settings block and were not re-declared on `MarketExpertInterface`. They are still READ (by exact key) from an expert's stored settings, so an operator who sets them gets the documented behaviour — but nothing declares them, nothing renders them, and an expert holding option structures without them has its lifecycle pass abort loudly.
