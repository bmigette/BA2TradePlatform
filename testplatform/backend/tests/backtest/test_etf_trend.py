...
def test_bad_basket_aborts_backtest_instead_of_being_silently_skipped(settings):
    from ba2_providers.fmp_common import FMPHistoryCacheMiss
    expert = object.__new__(ETFTrend)
    provider = SimpleNamespace(get_ohlcv_data=lambda *a, **kw: None)
    bundle = SimpleNamespace(ohlcv=lambda: provider)
    with pytest.raises(FMPHistoryCacheMiss, match="complete basket"):
        expert.analyze_as_of(datetime(2020, 2, 3), BacktestContext(
            providers=bundle, settings=settings, extra={"symbol": "AAA"}))
...
def test_real_engine_opens_selected_fund_through_shared_rules(monkeypatch):
    ...
    # The engine also evaluates warmup bars. Supply a full prior month for its
    # first January warmup scan, rather than relying on swallowed missing data.
    monkeypatch.setattr(fixtures, "_BASE_START", date(2023, 1, 2))
    monkeypatch.setattr(fixtures, "_N_BARS", 400)
    monkeypatch.setattr(fixtures, "_PRICE_ROWS", {
        s: fixtures._build_price_rows(s) for s in ["AAPL", "MSFT"]})
