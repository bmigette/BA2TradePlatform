...
def test_resizing_down_shrinks_the_pool_and_rebuilds_it(worker):
    ...
    out = worker.pool_resize({"workers": 6}, None, authorization="x")
...
def test_resizing_up_is_allowed_too(worker):
    worker.pool_resize({"workers": 4}, None, authorization="x")
    out = worker.pool_resize({"workers": 10}, None, authorization="x")
...
def test_resizing_to_the_same_size_is_a_noop(worker):
    ...
    out = worker.pool_resize({"workers": 12}, None, authorization="x")
...
def test_a_busy_worker_refuses_to_resize(worker):
    ...
    _busy(worker, 3)
    out = worker.pool_resize({"workers": 4}, None, authorization="x")
...
def test_completed_jobs_do_not_block_a_resize(worker):
    ...
    out = worker.pool_resize({"workers": 5}, None, authorization="x")
...
def test_a_nonsense_size_is_rejected_without_touching_the_pool(worker, bad):
    with pytest.raises(Exception):
        worker.pool_resize({"workers": bad}, None, authorization="x")
...
def test_resize_updates_what_health_advertises(worker):
    ...
    worker.pool_resize({"workers": 3}, None, authorization="x")
    assert worker.health(None, authorization="x")["capacity"] == 3
...
def test_capacity_gate_follows_the_new_size(worker, monkeypatch):
    ...
    worker.pool_resize({"workers": 2}, None, authorization="x")
    _busy(worker, 2)
...
def test_resize_is_clamped_to_the_daemons_own_ceiling(worker):
    ...
    out = worker.pool_resize({"workers": 40}, None, authorization="x")
...
def test_health_reports_the_ceiling_alongside_the_current_size(worker):
    ...
    it may grow it back."""
    worker.pool_resize({"workers": 4}, None, authorization="x")
    h = worker.health(None, authorization="x")
...
def test_a_shrunk_pool_can_be_grown_back(worker):
    ...
    worker.pool_resize({"workers": 6}, None, authorization="x")
    assert worker._CAPACITY == 6
    out = worker.pool_resize({"workers": 12}, None, authorization="x")
