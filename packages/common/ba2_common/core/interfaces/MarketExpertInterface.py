...
class MarketExpertInterface(ExtendableSettingsInterface):
    ...
    # BYPASS marker: when True the expert produces portfolio weight targets and
    # rebalances via its OWN portfolio manager, so the backtest/live engine must
    # route its analyze_as_of targets DIRECTLY to that manager and SKIP the shared
    # TradeActionEvaluator/TradeConditions + TradeRiskManagement + position_sizing
    # pipeline (and the optimizer must exclude rm:*/cond:/exit:/tp/sl from the
    # search space). Read via getattr(expert, "bypasses_classic_rm", False).
    # Base default is False; only weight-target experts (e.g. FactorRanker) set it.
    bypasses_classic_rm: bool = False
    ...
    def _gather(self, providers: "ProviderBundle", as_of: Optional[datetime]) -> Dict[str, Any]:
        ...
        raise NotImplementedError(
            f"{type(self).__name__} has not implemented _gather (Phase 1)"
        )
    ...
    def _process(self, data_bundle: Dict[str, Any], settings: Dict[str, Any],
        ...
        """
        raise NotImplementedError(
            f"{type(self).__name__} has not implemented _process (Phase 1)"
        )
    ...
    def _live_providers(self) -> "ProviderBundle":
        """Build a LiveProviderBundle from the host-injected provider resolver.

        ba2_common never imports ba2_providers; the host wires the resolver
        (ba2_common.core.TradeConditions.set_provider_resolver(get_provider)) at
        startup. For the Phase-1 golden harness the test injects get_provider via
        the same seam. Used by every expert's live run_analysis to feed _gather.
        """
        from ba2_common.core.backtest_context import LiveProviderBundle
        from ba2_common.core.TradeConditions import _get_provider
        return LiveProviderBundle(lambda category, name, **kw: _get_provider(category, name, **kw))
...
class BacktestInterface(Protocol):
    """Structural protocol for anything that can be backtested via analyze_as_of.

    Every refactored MarketExpertInterface subclass satisfies this protocol because
    MarketExpertInterface defines analyze_as_of. The backtest engine type-hints on
    this protocol so it never imports a concrete expert.
    """
    def analyze_as_of(self, as_of: datetime, context: BacktestContext) -> Recommendation: ...
