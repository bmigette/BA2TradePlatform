...
class TradeRiskManagement:
    ...
    def __init__(self, indicator_provider=None, as_of=None):
        ...
        Args:
            indicator_provider: optional MarketIndicatorsInterface impl used by the
                risk_atr sizing mode to fetch the latest ATR when an order has no
                explicit stop price. ba2_common must not import ba2_providers, so the
    ...
    def review_and_prioritize_pending_orders(self, expert_instance_id: int) -> List[TradingOrder]:
        ...
        try:
            ...
            # Get the expert instance with settings (resolved via the injected host resolver)
