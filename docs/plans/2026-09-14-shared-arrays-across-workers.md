...
0. The `TEST_APP_VERSION` bump is load-bearing for the cache-sync change: `POST /workers/{id}/sync-cache` has no `ensure_synced`, so never use it against a worker still on pre-28880c48 code (its manifest would still list `_derived`, and the master's would not — `diff_stale` then prunes that worker's derived cache on every push).
