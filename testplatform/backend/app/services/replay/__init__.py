"""Offline replay of recorded live analyses (spec step 3).
...
outcome. This package reads one exported session bundle back and answers two of
the spec's three comparison capabilities, offline:
...
``historical`` and ``decision`` are LATER deliveries and are reported as
``not_run`` -- never as absent rows, and never rolled into a percentage.
